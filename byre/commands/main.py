...
class MainCommand(ConfigurableGroup):
    ...
    def configure(self, config: GlobalConfig):
        self._scorer = scoring.Scorer(
            free_weight=config.optional(float, 1.0, "scoring", "free_weight"),
            cost_recovery_days=config.optional(
                float, 7.0, "scoring", "cost_recovery_days"
            ),
            removal_exemption_days=config.optional(
                float, 15.0, "scoring", "removal_exemption_days"
            ),
        )
        planner_configs = []
        for disk in config.require(dict, "planning").keys():
            max_total_size = utils.convert_iec_size(
                config.optional(str, "0", "planning", disk, "max_total_size")
            )
            planner_configs.append(
                planning.PlannerConfig(
                    max_total_size=max_total_size,
                    max_download_size=utils.convert_iec_size(
                        config.optional(
                            str,
                            f"{max_total_size / 50} B",
                            "planning",
                            disk,
                            "max_download_size",
                        )
                    ...
                    download_dir=os.path.realpath(
                        config.require(str, "planning", disk, "download_dir")
                    ),
                )
            )
        self._planner = planning.Planner(planner_configs)
        self._store = storage.TorrentStore(
            config.require(str, "qbittorrent", "cache_database")
        )
        self._config = config
    ...
    @click.option(
        "-d", "--dry-run", is_flag=True, help="计算种子选择结果，但不添加种子到本地"
    )
    @click.option(
        "-p",
        "--print",
    ...
    def fix(self, dry_run: bool):
        ...
        pending = [
            t
            for t in self.bt.api.list_torrents([], wants_all=True, site="byr")
            if t.seed_id == 0
        ]
        if len(pending) == 0:
            _info("所有种子均已经正确命名")
            return
        for kind in [
            UserTorrentKind.SEEDING,
            UserTorrentKind.COMPLETED,
            UserTorrentKind.LEECHING,
        ...
        if dry_run:
            _info(
                "以上计划未被实际运行；若想实际重命名，请去除命令行 -d / --dry-run 选项"
            )
        else:
    ...
    def stat(self):
        ...
        click.echo(
            f"当前种子总上传量 {S(uploaded)}，最近一次重启后上传量 {S(uploaded_session)}"
        )
        total, _, _ = self.planner.merge_torrent_info(
            [(t, 0) for t in local], self.store
        )
        for config in self.planner.configs:
            click.echo(f"下载目录：{config.download_dir}")
            click.echo(
                f"    当前本地种子已用空间 {S(total.get(config.download_dir, 0.))}，"
    ...
    @click.argument(
        "seed", type=click.STRING, metavar="<北邮人链接或是种子 ID>", nargs=-1
    )
    @click.option(
        "-a",
        "--at",
    ...
    @click.option(
        "-d", "--dry-run", is_flag=True, help="计算种子调整结果，但不添加种子到本地"
    )
    ...
    @click.option(
        ...
        "--exists",
        is_flag=True,
        help="告诉 qBittorrent 文件已经下载完毕并让其跳过哈希检查",
    )
    @click.option(
        "-s",
        "--same",
    ...
    def download_one(
        ...
        dry_run: bool,
        paused: bool,
        exists: typing.Union[bool, LocalTorrent],
        same: str,
    ):
        ...
        if (
            self.download(
                [api.torrent(seed_id) for seed_id in seed_ids],
                dry_run,
                paused=paused,
                exists=exists,
            )
    ...
    @click.command
    @click.option(
        "-d", "--dry-run", is_flag=True, help="计算种子下载结果，但不添加种子到本地"
    )
    def hitchhike(self, dry_run: bool):
        ...
        byr_torrents = [
            t for t in torrents if t.site == "byr" and t.torrent.amount_left == 0
        ]
        byr_torrents.sort(key=lambda t: t.torrent.size)
        existing_seeds: dict[str, list[int]] = dict(
            (name, []) for name in self.sites.keys()
        )
        for t in torrents:
            if t.site != "byr":
        ...
        for name, existing in existing_seeds.items():
            ...
            page.extend(
                api.list_torrents(page=0, sorted_by=NexusSortableField.SEEDER_COUNT)
            )
            ...
            for torrent in page:
                ...
                for local in byr_torrents[i:]:
                    ...
                    if self._match_words(local.torrent.name, torrent.title):
                        ...
                        if self._torrent_files_exact_match(
                            local, remote_content, torrent
                        ):
    ...
    def download(
        self,
        targets: typing.Optional[list[TorrentInfo]] = None,
        dry_run=False,
        print_scores=False,
        free_only=False,
        paused=False,
        exists: typing.Union[LocalTorrent, bool] = False,
        site="byr",
    ):
        if site == "byr":
            api = self.byr.api
        ...
        estimates, groups = self.planner.estimate(
            scored_local, removable, downloadable, self.store, exists=bool(exists)
        )
        summary = []
        for config in self.planner.configs:
        ...
        if dry_run:
            _info(
                "以上计划未被实际运行；若想开始下载，请去除命令行 -d / --dry-run 选项"
            )
        else:
            ...
            for t, directory in downloadable:
                ...
                if isinstance(exists, LocalTorrent):
                    if not self._torrent_files_exact_match(exists, torrent, t):
                        ...
                        continue
                self.bt.api.add_torrent(
                    torrent, t, directory, paused=paused, exists=exists
                )
    ...
    def _match_against_remote(
        self, pending: list[LocalTorrent], remote: list[TorrentInfo]
    ):
        ...
        return [
            (
                "/".join(
                    (
                        root,
                        *(cast(bytes, segment).decode() for segment in file[b"path"]),
                    )
                ),
    ...
    def _match_words(a: str, b: str):
        ...
        matches = set(s.lower() for s in separator.split(a) if len(s) > 3) & set(
            s.lower() for s in separator.split(b) if len(s) > 3
        )
        if len(matches) != 0:
    ...
    @classmethod
    def _torrent_files_exact_match(
        cls, local: LocalTorrent, remote_torrent: bytes, remote: TorrentInfo
    ):
        local_files = dict(
            (cast(str, file.name), cast(int, file.size)) for file in local.torrent.files
        )
        remote_files = dict(cls._extract_torrent_files(remote_torrent))
        if local_files == remote_files:
            _debug(
                "文件详情匹配：均 %d 文件，文件路径、文件大小完全一致", len(local_files)
            )
